...
import { ArrowLeft, ClipboardCheck, Eye, Filter } from "lucide-react";
...
export default async function TeamDrilldownPage({
  ...
  const inProgressPct = Math.round((stats.inProgress / stats.totalTasks) * 100);
  ...
  return (
    <div className="min-h-screen bg-[#F5F6F8] text-[#16233F]">
      <header className="flex h-16 items-center justify-between border-b border-[#E5E9F0] bg-white px-6">
        <span className="text-xl font-bold tracking-tight">Acentura</span>
      </header>

      <main className="mx-auto max-w-[1400px] px-8 py-8">
        <Link
          href="/executive-summary"
          className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-[#16233F] hover:underline"
        >
